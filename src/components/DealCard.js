...
export default function DealCard({ deal, view = 'grid', delay = 0 }) {
  ...
  const amazonLink = deal.comparePrices
    ? (deal.comparePrices.find(function(p) { return p.retailer === 'amazon' }) || {}).url || deal.affiliateUrl
    : deal.affiliateUrl
  ...
  function handleWishlist(e) {
    ...
    try {
      ...
      if (!exists) {
        ...
        alert('Added ' + deal.shortName + ' to your wishlist! Visit Wishlists to manage your lists.')
      } else {
  ...
  if (view === 'list') {
    return (
      <div className="fade-up" style={{ animationDelay: delay * 0.06 + 's', background: 'white', borderBottom: '1px solid rgba(26,26,26,0.08)', padding: '20px 0', display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
        <div style={{ width: '80px', height: '80px', background: '#F5F0E8', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
          <img src={deal.imageUrl} alt={deal.name} style={{ width: '100%', height: '100%', objectFit: 'contain', padding: '8px' }}
            onError={function(e) { e.target.style.display='none'; e.target.parentNode.innerHTML='<div style="font-size:32px;display:flex;align-items:center;justify-content:center;width:100%;height:100%">' + deal.emoji + '</div>' }} />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flexShrink: 0 }}>
          <a href={amazonLink} target="_blank" rel="noopener noreferrer"
            style={{ fontFamily: 'Jost, sans-serif', fontSize: '11px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'white', background: '#185FA5', padding: '8px 14px', textDecoration: 'none', whiteSpace: 'nowrap', textAlign: 'center', display: 'block' }}>
            Buy on Amazon
          </a>
          <Link href={'/product/' + deal.id}
            style={{ fontFamily: 'Jost, sans-serif', fontSize: '11px', fontWeight: 400, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#185FA5', border: '1px solid #185FA5', padding: '7px 14px', textDecoration: 'none', whiteSpace: 'nowrap', textAlign: 'center', display: 'block' }}>
            Compare stores
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="fade-up" style={{ animationDelay: delay * 0.06 + 's', background: 'white', cursor: 'pointer', transition: 'transform 0.25s ease', display: 'flex', flexDirection: 'column' }}
      ...
      <div style={{ aspectRatio: '4/3', background: '#F5F0E8', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
        <img src={deal.imageUrl} alt={deal.name}
          ...
          onError={function(e) {
            ...
            fallback.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:64px'
            fallback.textContent = deal.emoji
        ...
        <button onClick={handleWishlist} title="Add to wishlist"
          style={{ position: 'absolute', top: '12px', right: '12px', background: 'white', border: 'none', width: '34px', height: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#185FA5', zIndex: 2, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
          <Heart size={15} />
        </button>
      </div>
      <div style={{ padding: '16px 16px 20px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <Link href={'/product/' + deal.id}
          style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '20px', fontWeight: 400, color: '#1A1A1A', textDecoration: 'none', display: 'block', lineHeight: '1.3', marginBottom: '10px' }}>
          {deal.name}
        </Link>
        ...
        <a href={amazonLink} target="_blank" rel="noopener noreferrer"
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#185FA5', color: 'white', textDecoration: 'none', padding: '12px 16px', fontFamily: 'Jost, sans-serif', fontSize: '12px', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px' }}>
          Buy on Amazon
        </a>
        <Link href={'/product/' + deal.id}
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Jost, sans-serif', fontSize: '11px', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888', textDecoration: 'none', borderTop: '1px solid rgba(26,26,26,0.06)', paddingTop: '8px' }}>
          Compare all stores
        </Link>
      </div>
    </div>
