...
export default function GuideDetailPage({ params }) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#0A0E1A' }}>
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto', padding: '120px 24px', textAlign: 'center' }}>
          <h1 style={{ fontFamily: 'DM Serif Display, serif', fontSize: '42px', color: '#F0F4FF', marginBottom: '16px' }}>Guide not found</h1>
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '16px', color: '#94A3B8', marginBottom: '32px' }}>This guide doesn't exist or may have moved.</p>
          <Link href="/guides" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#00D084', color: '#0A0E1A', textDecoration: 'none', padding: '12px 28px', borderRadius: '8px', fontFamily: 'DM Sans, sans-serif', fontSize: '13px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
            Back to Guides
          </Link>
        </div>
        <Footer />
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0E1A' }}>
      ...
      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg, #0D1420 0%, #0A0E1A 100%)', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '56px 24px 48px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px' }}>
            <Link href="/guides" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontFamily: 'DM Sans, sans-serif', fontSize: '13px', color: '#475569', textDecoration: 'none' }}>
          ...
          <h1 style={{ fontFamily: 'DM Serif Display, serif', fontSize: 'clamp(28px, 5vw, 46px)', color: '#F0F4FF', lineHeight: 1.15, marginBottom: '16px' }}>{guide.title}</h1>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '56px 24px' }}>
        ...
        <div style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', borderLeft: '3px solid #3B82F6', borderRadius: '10px', padding: '16px 20px', marginBottom: '48px' }}>
          ...
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '13px', color: '#94A3B8', lineHeight: 1.6 }}>CloudPriceDeals earns a commission on purchases made through links on this page. This never affects our recommendations.</p>
        </div>
        ...
        {guide.content.map(function(section, i) {
          return (
            <div key={i} style={{ marginBottom: '40px' }}>
              <h2 style={{ fontFamily: 'DM Serif Display, serif', fontSize: '26px', color: '#F0F4FF', marginBottom: '14px', lineHeight: 1.25 }}>{section.heading}</h2>
              <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '16px', color: '#CBD5E1', lineHeight: 1.85 }}>{section.body}</p>
            </div>
          )
        })}
        ...
        {relatedDeals.length > 0 && (
          <div style={{ marginTop: '64px', paddingTop: '48px', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
            <h2 style={{ fontFamily: 'DM Serif Display, serif', fontSize: '28px', color: '#F0F4FF', marginBottom: '24px' }}>Current deals mentioned in this guide</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {relatedDeals.map(function(deal) {
                var pct = Math.round(((deal.originalPrice - deal.price) / deal.originalPrice) * 100)
                return (
                  <a key={deal.id} href={deal.affiliateUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px 20px', textDecoration: 'none', gap: '12px' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '14px', fontWeight: 600, color: '#F0F4FF', marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{deal.name}</p>
                      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '12px', color: '#475569' }}>Amazon · Free Prime shipping</p>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
                      <span style={{ fontFamily: 'DM Serif Display, serif', fontSize: '20px', color: '#00D084' }}>${deal.price}</span>
                      <span style={{ background: 'rgba(0,208,132,0.12)', color: '#00D084', border: '1px solid rgba(0,208,132,0.25)', fontFamily: 'DM Sans, sans-serif', fontSize: '11px', fontWeight: 700, padding: '3px 8px', borderRadius: '100px' }}>-{pct}%</span>
        ...
        <div style={{ marginTop: '64px', textAlign: 'center' }}>
          <Link href="/guides" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'transparent', color: '#00D084', border: '1px solid rgba(0,208,132,0.3)', textDecoration: 'none', padding: '12px 28px', borderRadius: '8px', fontFamily: 'DM Sans, sans-serif', fontSize: '13px', fontWeight: 600 }}>
